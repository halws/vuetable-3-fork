  ...
  computed: {
    ...
    useDetailRow() {
      if (
        this.tableData &&
        this.tableData[0] &&
        this.detailRowComponent !== "" &&
        typeof this.tableData[0][this.trackBy] === "undefined"
      ) {
        this.warn(
          "You need to define unique row identifier in order for detail-row feature to work. Use `track-by` prop to define one!"
        );
        return false;
      }
  ...
  methods: {
    ...
    renderTitle(field) {
      ...
      if (
        ...
        let iconTag = this.showSortIcons
          ? this.renderIconTag(
    ...
    getDefaultSortParam() {
      ...
      for (let i = 0; i < this.sortOrder.length; i++) {
        let fieldName =
          typeof this.sortOrder[i].sortField === "undefined"
    ...
    checkCheckboxesState(fieldName) {
      ...
      let idColumn = this.trackBy;
      let selector =
        "th.vuetable-th-checkbox-" + idColumn + " input[type=checkbox]";
    ...
    gotoPage(page) {
      if (
        ...
        page > 0 && page <= this.tablePagination.last_page
      ) {
    ...
    onCellRightClicked(dataItem, field, event) {
      this.$emit(
        this.eventPrefix + "cell-rightclicked",
        dataItem,
        field,
        event
      );
